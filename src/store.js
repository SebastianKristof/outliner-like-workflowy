...
export function createStore() {
  ...
  const ensureSelection = (id, caretOffset, mode = 'caret') => {
    const previous = state.ui.selection;
    const next = { anchorId: id, caretOffset, mode };
    if (
      previous &&
  ...
  const store = {
    ...
    focusItem(id, caretOffset, mode = 'caret') {
      if (ensureSelection(id, caretOffset, mode)) {
        notify();
      }
    },
    toggleCollapse(id) {
    ...
    insertNodes(parentId, afterId, nodes) {
      ...
      if (created.length && ensureSelection(created[created.length - 1], 0)) {
        notify();
      }
      return created;
    },
